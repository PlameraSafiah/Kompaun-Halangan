...
'============================== if click button hantar ======================================
	...
	if p = "Hantar" and (kompaun <> "" or no_akaun <> "") then			
...
'====================================== if click simpan =====================================
	...
	if kompaun = "" or no_akaun = "" then
		papar
		response.write "<script language=""VBScript"">"
		response.write " MsgBox ""Sila Masukkan No Kompaun atau No Akaun"", vbInformation, ""Perhatian!"" "
		response.write "</script>"	%>
	...
	if not sb.eof then
		...
		n = " select no_kompaun from kompaun.halangan where no_kompaun = '"&kompaun&"' or no_akaun='"&no_akaun&"' "
  		Set sn = objConn.Execute(n)
	
	
		if not sn.eof then
			d = "     update kompaun.halangan set nama = '"& nama &"',kp = '"& kp &"',alamat1 = '"& alamat1 &"', "
			d = d & " alamat2 = '"& alamat2 &"',alamat3 = '"& alamat3 &"', no_kenderaan = '"&kenderaan&"',jenis_kenderaan = '"&jenis&"', "
			...
			d = d & " where no_kompaun = '"& kompaun &"' or no_akaun='"&no_akaun&"' "
		    ...
		    b2 = "insert into hasil.bil(no_akaun, nama, alamat1, alamat2, alamat3, kategori,jenis,"
		    b2 = b2 & " no_rujukan,perkara1,"
		    b2 = b2 & " amaun,tkh_masuk,siri,tahun,akaun,tkh_entry,penyedia) " 
		    b2 = b2 & " values ('"&nwakaun&"','"&nama&"','"&alamat1&"','"&alamat2&"','"&alamat3&"','45','"&jenis&"', "
			...
			d = d & " no_kenderaan,jenis_kenderaan,cukai_jalan,akta,kesalahan,tkh_kompaun,tkh_input, masa, tempat, "
			...
			d = d & " '"& alamat2 &"' , '"& alamat3 &"' , '"& kenderaan &"' , '"& jenis &"' , '"& cukai &"' , "
			d = d & " '"& akta &"', '"& salah &"', "
...
<input type="text" style="font-family: Trebuchet MS; font-size: 10pt;" name="no_akaun" size="15" value="<%=no_akaun%>" maxlength="13" onFocus="nextfield='B';">
atau 
</td>
...
document.komp.kompaun.focus()
  ...
  <% end sub 
	...
	'penambahan bagi memapar status kompaun , tarikh notis dihantar ---> nadia (09092016) 	
  	s = " 	select rowid, no_kompaun,upper(nama)nama,no_akaun, "
	s = s & " upper(alamat1) alamat1, upper(alamat2) alamat2, upper(alamat3) alamat3, kp, to_char(tkh_notis1,'dd/mm/yyyy') tkh_notis1, "
	s = s & " no_kenderaan,cukai_jalan, akta,kesalahan,kenderaan_berat,to_char(tkh_batal,'dd/mm/yyyy') tkh_batal,"
	s = s & " to_char(tkh_kompaun,'dd/mm/yyyy') tkh_kompaun,to_char(masa) masa,tempat,tempat1,jenis_kenderaan, "
	s = s & " daerah,nvl(amaun,0)amaun,pengeluar_kompaun,daerah,no_resit,nvl(amaun_bayar,0)amaun_bayar, status_kompaun ,"
	s = s & " to_char(tkh_bayar,'dd/mm/yyyy')tkh_bayar,no_kontena,butir_kesalahan , catitan "
	s = s & " from kompaun.halangan "
	s = s & " where no_kompaun = '"& kompaun &"' or no_akaun='"&no_akaun&"' "
	...
	if not gq.eof then
		...
		catitan = gq("catitan")
		status_kompaun = gq("status_kompaun")
		tkh_notis1 = gq("tkh_notis1")
		tkh_batal = gq("tkh_batal")
		jenis = gq("jenis_kenderaan")
  ...
  <% end if 
	...
	<tr> 
    <td width="127" bgcolor="<%=color1%>"><font  color="#FFFF00">No Kompaun</font></td>
    <td colspan="2" bgcolor="<%=color2%>"><font color="red"><b><%=kompaun%></b></font></td>
    <td bgcolor="<%=color2%>">&nbsp;</td>
    <td colspan="2" bgcolor="<%=color2%>">&nbsp;</td>
  </tr>
...
<tr> 
      <td  bgcolor="936975" width="127"><font  color="#FFFF00">Jenis Kenderaan</font></td>
      <td bgcolor="lightgrey"> <font face="Arial" size="1"> 
      <select name="jenis" onFocus="nextfield='jenama';">
      <%if jenis <> "" then
        jj = " select decode('"& jenis &"','1','Motosikal','2','Kereta','3','Van/Lori Kecil/4WD','4','Lori Besar/Bas',null) "
        jj = jj & " njenis from dual "
        Set Rsjj = objConn.Execute(jj)
	 ...
	 %>
          <option value="<%=jenis%>" selected><%=njenis%></option>
          <option value="2"> Kereta</option>
          <option value="1"> Motosikal</option>
          <option value="3"> Van/Lori Kecil/4WD</option>
          <option value="4"> Lori Besar/Bas</option>
          <%else if njenis = "" then %>
          <option value="2"> Kereta</option>
          <option value="1"> Motosikal</option>
          <option value="3"> Van/Lori Kecil/4WD</option>
...
<!--  </td>-->
  ...
  <tr> 
    <td width="127" bgcolor="936975"><font  color="#FFFF00">Tarikh Kompaun</font></td>
    <td bgcolor="lightgrey" colspan="2"> 
  ...
  <tr> 
    ...
    <td colspan="5" bgcolor="lightgrey"> 
    <%
	if dae="04" then dae1="SPT" end if
	if dae="05" then dae1="SPS" end if
	if dae="06" then dae1="SPU" end if
	%>  
    
      <select name="dae" size="1" onKeyDown="if(event.keyCode==13) event.keyCode=9;" style="font-family: Trebuchet MS; font-size: 10pt;">
        <option value = "">Daerah</option>
        <% if dae <> "" then%>
        <option value="<%=dae%>" selected><%=dae%>-<%=dae1%></option>
        <% end if%>
  ...
  <tr> 
   ...
   <tr> 
    <td width="127" bgcolor="936975"><font color="#FFFF00">Status Mahkamah</font></td>
    <td colspan="5" bgcolor="lightgrey"> 
    <!---------------------------TAMBAH STATUS BATAL KES MAHKAMAH - 04042017---------------------------->
	<%if status_kompaun="M" then%><font color="red"><b>KES MAHKAMAH</b></font>
	<%end if%>
    <%if status_kompaun<>"M" then%><font color="red"><b>Tiada</b></font>
	<%end if%>
    <!--------------------------------------------------------------------------------------------------> 
    </td>
  </tr>
   ...
   <tr> 
    <td width="127" bgcolor="936975"><font color="#FFFF00">Batal Mahkamah</font></td>
    <td colspan="5" bgcolor="lightgrey"> 
    <!---------------------------TAMBAH STATUS BATAL KES MAHKAMAH - 04042017---------------------------->
	<%if status_kompaun="M" and tkh_batal <> "" then%><font color="red"><b>YA</b></font>
	<%end if%>
    <!--------------------------------------------------------------------------------------------------> 
    </td>
  </tr>
   <tr> 
    <td width="127" bgcolor="936975"><font color="#FFFF00">Tarikh Notis</font></td>
    <td colspan="5" bgcolor="lightgrey"><%=tkh_notis1%></td>
  </tr>
  <tr> 
...
<input type="text" style="font-family: Trebuchet MS; font-size: 10pt;" name="no_pekerja" size="5" value="<%=no_pekerja%>" maxlength="5" onFocus="nextfield='B1';">
      -&nbsp; <font color="#000000"><%=napek%>&nbsp;</font><font color="red">*</font>
 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color="red">*</font>Ruangan ini <b>WAJIB</b> diisi
    </td>
  </tr>
  ...
  <tr> 
    ...
    <%if status_kompaun = "P" then%>
      <font color="red"><b>KOMPAUN INI TELAH DIBAYAR</b></font>
    <%end if%>
    <%if status_kompaun = "B" then%>
      <font color="red"><b>KOMPAUN INI TELAH DIBATAL</b></font>
    <%end if%>
    </td>
  </tr> 
  <tr align="left"> 
  ...
  <tr bgcolor="936975" align="center" valign="top"> 
    ...
    <!----tambah untuk papar bil bagi kes batal mahkamah---------->
     <%if tkh_batal <> "" and status_kompaun = "M" then%> 
     <form method="Post"  action="hg111.asp" name=hg111b>
	 <input type="hidden" name="rowid" value="<%=rowid%>" >
     <input type="hidden" name="kompaunh" value="<%=kompaun%>" >
	 <input type="hidden" name="no_akaun" value="<%=no_akaun%>" >
    ...
    <%'if status_kompaun = "M" and tkh_batal = "" then%>
