...
'====================================== if click simpan =====================================
	...
	if not sb.eof then
		...
		if not sn.eof then
			...
			d = d & " kesalahan = '"& salah &"',butir_kesalahan = '"&butirsalah&"', "
			...
			aa = aa & " where no_akaun like '76439'||'%' and length(no_akaun)=11 "
			aa = aa & " and to_char(tkh_entry,'yyyy') = '"&thn&"' and substr(no_akaun,6,1)='"&digithn&"' "
		...
		set sb2 = objconn.execute(b2)
			...
			d = d & "  pengeluar_kompaun, amaun,status_kompaun,daerah,butir_kesalahan,no_kontena,kenderaan_berat,tempat1) values "
			...
			d = d & " '"&butirsalah&"','"&kontena&"','"& kenderaan_berat &"','"&tempat1&"') "
			Set objRs2 = objConn.Execute(d)			
