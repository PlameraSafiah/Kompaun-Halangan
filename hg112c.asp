...
<form method="POST" action="hg112c.asp">
<% 
	Set objConn = Server.CreateObject("ADODB.Connection")
	objConn.Open "dsn=12c;uid=majlis;pwd=majlis;"
	
	kompaund = request.querystring("nokomd")
	kompaunh = request.querystring("nokomh")
	
	akaund = request.querystring("noakd")
	akaunh = request.querystring("noakh")
  ...
  '    =======================================     SELECT   ================================	
		...
		if kompaund<>"" then
		d = d & " no_kompaun between '"& kompaund &"' and '"& kompaunh &"' "
		end if
		if akaund <> "" then
		d = d & " no_akaun between '"& akaund &"' and '"& akaunh &"' "
		end if
		
		d = d & " and status_kompaun = 'N' and tkh_bayar is null "
		...
		l = "select butir_kesalahan from kompaun.halangan where no_kompaun = '"& sd("no_kompaun") &"' or no_akaun = '"& sd("no_akaun") &"'"
		set rsl = objconn.execute(l)
		catitan = rsl("butir_kesalahan")
		
		p = " update kompaun.halangan set cetak_notis = 'Y', tkh_notis1 = to_date('"&tkhdd&"','dd/mm/yyyy') "
		p = p & " where no_kompaun = '"&sd("no_kompaun")&"'  or no_akaun = '"& sd("no_akaun") &"' "
		set sp = objconn.execute(p)		
		
		m = " select initcap('"&replace(catitan,"'","")&"') catitan from dual "
		...
		if not sc.eof then			jenis_salah = sc("terang")
...
<br>
  ...
  <table width=78% cellspacing="0" cellpadding="0" style="font-family: Times New Roman; font-size: 11pt;" align="center">
    <tr> 
     ...
     <tr> 
      <td width=24%>&nbsp;NO AKAUN</td>
      <td width=76%>: <%=sd("no_akaun")%></td>
    </tr>
    <tr> 
  ...
  <table width="78%" border="0" cellspacing="0" cellpadding="1" align="center" style="font-family: Times New Roman; font-size: 11pt;">
    ...
    <tr>
      <td> 
        <p align="justify" id=tengah>3. Pembayaran boleh dibuat dengan 
          ...
          dan di palang dengan perkataan <b>"AKAUN PENERIMAAN SAHAJA"</b>.</p>
      </td>
    </tr>
	 <tr>
      <td>&nbsp;</td>
    ...
    <tr> 
      <td><p>Sekian, terima kasih.</p></td>
    </tr>
    <tr> 
      <td>        <p ><br>
          <br>
          <br>
          <b>(NAZRI BIN ABDUL WAHAB)</b> <br>
          Penolong Pengarah Direktorat Penguatkuasaan, <br>
