...
<form name=komp method="POST" action="hg187.asp"> 
 <%	 
   p = Request.form("b")
   p1 = Request.form("b1")
...
%> 
 ...
 '============================================== SUB CARI ====================================
	  sub cari
		d = " select a.rowid,a.no_kompaun,initcap(a.nama) nama,b.no_saman, "
		d = d & " to_char(a.tkh_undang,'dd/mm/yyyy') tkh_undang,a.no_akaun,a.tempat,a.tempat1,a.tkh_kompaun, "
		d = d & " to_char(b.tarikh_saman,'dd/mm/yyyy') tkh_saman,b.keputusan, "
		d = d & " to_char(a.tkh_notis1,'dd/mm/yyyy')tkh_notis3,a.akta,a.kesalahan "
		d = d & " from kompaun.halangan a,kompaun.mahkamah b "
		d = d & " where a.tkh_undang between to_date('"& tkh1 &"','dd/mm/yyyy') "
		d = d & " and  to_date('"& tkh2 &"','dd/mm/yyyy') "
		d = d & " and  a.no_kompaun = b.no_kompaun "
		d = d & " and  b.no_saman is not null "
		d = d & " order by a.no_kompaun "
...
%><br>
  <table width="100%" align="center" cellspacing="1">
    <tr bgcolor="<%=color1%>" align="center" style="font-family: Trebuchet MS; font-size: 10pt; color:yellow"> 
      ...
      <td width="12%">No Saman</td>
      <td width="12%">Tarikh Sebutan</td>
      <td width="12%">Keputusan Kes</td>
    </tr>
    <% 	bil = 0  	
 	Do while not sd.EOF
   	bil = bil + 1
%>
    <tr bgcolor="<%=color2%>" align="center" style="font-family: Trebuchet MS; font-size: 10pt;"> 
      ...
      <td width="12%"><%=sd("no_saman")%></td>
      <td width="12%"><%=sd("tkh_saman")%></td>
      <td width="12%"><%=sd("keputusan")%></td>
