...
function batal_sekatan(b)
    {  
       alert (b+" Sekatan Dibatalkan !!! ");
		return(true);
    }
</script>
...
<%	
	...
	b1batalsek = Request.Form("b1batalsek")
...
'response.write r1
    ...
    '************************************* PROSES BATAL SEKATAN ****************************************************
	
	
	if b1batalsek = "Batal Sekatan" then
	
	
	dd = "delete from kompaun.mahkamah where no_kompaun = '"& kompaun &"' or no_akaun= '"& noakaun &"' "
	''response.Write(dd)
	set objdd = objconn.execute(dd)
	
	
	 h = " select rowid ,no_akaun from kompaun.halangan where no_kompaun = '"& kompaun &"' or no_akaun= '"& noakaun &"'"
		set Rsh = objconn.execute(h)
		...
		''------status_kompaun = 'X' bermaksud kompaun mahkamah yang telah dibatalkan sekatan
		 
			r1 = " update kompaun.halangan set   "           	
			r1 = r1 & " tkh_notis1 = '' , " 
			r1 = r1 & " tkh_undang = '', " 
			r1 = r1 & " tkh_sekat = '', "
			r1 = r1 & " status_kompaun = 'X' "           
           	r1 = r1 & " where rowid = '"& rowa &"' "
			''response.Write(r1)
          	 Set Rsr1 = objConn.Execute(r1)
		   
		   
		   u = " update hasil.bil set perkara5 = '' , tarikh_notis1 = '' where no_akaun = '"& no_ak &"' "
        ...
        response.write "var timeID = setTimeout('batal_sekatan(""  "");',1) "
  ...
  if proses = "Save" then
     ...
     if tkh2s > tkhs then
        ...
        g = " select rowid from kompaun.mahkamah where no_kompaun = '"& kompaun &"' or no_akaun= '"& noakaun &"'"
		'response.Write(g)
		...
		if Rsg.eof then
           ...
           s =     "insert into kompaun.mahkamah (no_saman,no_saman2, tarikh_saman, no_kompaun,no_akaun, no_daftar, nama, kp, "
           ...
           s = s & " '"& kompaun &"','"& noakaun &"', '"& kereta &"', '"& nama &"', '"& kp &"', "
		...
		if not Rsh.eof then
			rowa = Rsh("rowid")
           	...
           	r1 = r1 & " nama = '"& nama &"',kp = '"& kp &"', no_akaun = '"& noakaun &"',"
		...
		'response.write r1
			...
			s =     "insert into kompaun.halangan (no_kenderaan,nama,kp,alamat1,alamat2,alamat3,tkh_notis1,status_kompaun,tkh_undang,tkh_sekat) "
...
'response.write noakaun
     if (kompaun = "" and noakaun= "") then
        ...
        sk = sk & " where no_kompaun = '"& kompaun &"' or no_akaun= '"& noakaun &"' "
...
<table width="95%" align="center" cellspacing=0 bgcolor="<%=color1%>" style="font-family: Trebuchet MS; font-size: 10pt;" cellpadding="1">
        <tr> 
          ...
          Kompaun</font></td>
          <td bgcolor="#CCCCCC"> <input type="text" name="kompaun" maxlength="11" value="<%=kompaun%>" size="15" onFocus="enter('B1','1');" >
         ...
         <tr> 
          <td width="19%" bgcolor="<%=color1%>">&nbsp;<font color="#FFFF00">No 
          Akaun</font></td>
          <td bgcolor="#CCCCCC"> <input type="text" name="noakaun" maxlength="13" value="<%=noakaun%>" size="15" onFocus="enter('B1','1');" >
...
'**************************************************************************************************
  ...
  d =     "select no_saman,no_saman2,to_char(tarikh_saman,'ddmmyyyy') tsaman,no_daftar,no_kompaun, "
  ...
  d = d & " where no_kompaun = '"& kompaun &"' or no_akaun= '"& noakaun &"' "
  ...
  e = e & " to_char(tkh_sekat,'ddmmyyyy') tkh_sekat,to_char(tkh_batal,'ddmmyyyy') tkh_batal,no_kompaun,"
  e = e & " to_char(tkh_notis1,'ddmmyyyy') tarikh_notis1 from kompaun.halangan "
  e = e & " where no_kompaun = '"& kompaun &"' or no_akaun= '"& noakaun &"'"
...
<%end if%>
      ...
      <tr> 
        ...
        <td width="75%" bgcolor="#CCCCCC"> 
          <input type="text" name="kompaun" maxlength="11" value="<%=kompaun%>" size="15" onFocus="enter('noakaun','1');" >
          &nbsp;
      ...
      <tr> 
        <td width="25%" bgcolor="<%=color1%>">&nbsp;<font color="#FFFF00">No 
          Akaun</font></td>
        <td width="75%" bgcolor="#CCCCCC"> 
          <input type="text" name="noakaun" maxlength="13" value="<%=noakaun%>" size="15" onFocus="enter('tundang','1');" >
          &nbsp;
         <!-- <input type="hidden" name="noakaun" value="<%'=noakaun%>" >--></td>
      </tr>
      <tr> 
  ...
  %>
      ...
      <tr> 
        <td colspan="2" align="center" bgcolor="#CCCCCC">
         ...
         <input type="submit" name="b1batalsek" value="Batal Sekatan" onFocus="enter('b1save','2');" > &nbsp;&nbsp;&nbsp;
