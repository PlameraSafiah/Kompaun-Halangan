...
<%	
	...
	if tarikh = "" then 
 		st = "select to_char(sysdate,'ddmmyyyy') tkh from dual "
    	set rsst = objconn.execute(st)     
     	tarikh = rsst("tkh")
	 end if 	
...
'***************************************	PROSES CARI	***************************************************
  ...
  if proses = "Cari" then
	 ...
	 elseif tarikh = ""  then
        ...
        sk = sk & " where no_kompaun = '"& kompaun &"' and to_char(tkh_undang,'ddmmyyyy') = '"& tarikh &"' "
        ...
        if rssk.eof then
           ...
           response.end
			...
			sk1 =      "select * from hasil.bil "
			sk1 = sk1 & " where no_rujukan = '"& kompaun &"' "
			set rssk1 = objconn.execute(sk1)
			
			if not rssk1.eof then

			   mahkamah = rssk1("mahkamah")
...
<table width="95%" align="center" cellspacing=0 bgcolor="<%=color1%>" style="font-family: Trebuchet MS; font-size: 10pt;" cellpadding="1">
		 <tr> 
          <td width="127" bgcolor="<%=color1%>"><font color="#FFFF00">&nbsp;No 
           Kompaun</font></td>
          <td bgcolor="#CCCCCC"><input name="kompaun" type="text" value="<%=kompaun%>" size="12" maxlength="11" onFocus="enter('tarikh','1');"> <font color="#FF0000">*</font>
             </td>
        </tr>
        <tr> 
          <td width="127" bgcolor="<%=color1%>">&nbsp;<font color="#FFFF00">Tarikh</font></td>
          <td bgcolor="#CCCCCC"> <input type="text" name="tarikh" maxlength="10" value="<%=tarikh%>" size="10" onFocus="enter('B1','1');" > <font color="#FF0000">*</font>
...
'**************************************************************************************************
  ...
  e = e & " where no_kompaun = '"& kompaun &"' and to_char(tkh_undang,'ddmmyyyy') = '"& tarikh &"' "
