...
<%	response.cookies("amenu") = "hg116.asp" 
    ...
    ftahun = Request.QueryString("ftahun")
	...
	if ftkhn <> "" then
		...
		tahun = request.querystring("ftahun")
	end if
  ...
  <table width="100%" cellpadding=0 cellspacing=0 bordercolor="#CCCCCC" bgcolor="<%=color2%>"> 
     ...
     <tr bgcolor="<%=color2%>" style="font-family: Trebuchet MS; font-size: 10pt;"> 
      ...
      <td width="33%" align="left"> 
        ...
        Hingga 
        <input name="tkhh" type="text" value="<%=tkhh%>" size="10" maxlength="10" onKeyUp="DateFormat(this,this.value,event,false,'3')" onBlur="DateFormat(this,this.value,event,true,'3')" onFocus="nextfield='tahun';">
         &nbsp;<B>ATAU</B> </td>
    ...
    <tr bgcolor="<%=color2%>" style="font-family: Trebuchet MS; font-size: 10pt;"> 
       <td width="40%" align="right">Tahun Notis Dikeluarkan : </td>
       <td width="60%" align="left">&nbsp;
      <input name="tahun" type="text" id="tahun" onFocus="nextfield='daerah';" onKeyDown="if(event.keyCode==13) event.keyCode=9;" value="<%=tahun%>" size="4" maxlength="4">
      </td>
       <td width="29%" align="left">&nbsp;</td>
    </tr> 
    ...
    <tr bgcolor="<%=color2%>" style="font-family: Trebuchet MS; font-size: 10pt;"> 
      ...
      <td width="33%" align="left"> 
        &nbsp;
      <select name="daerah" onFocus="nextfield='done';">
...
'================================= SUB FORM ====================================================
	sub form
		...
		d = d & " to_char(tkh_kompaun,'dd/mm/yyyy') tkh_kompaun, tkh_notis1,no_fail, daerah ,"
     	d = d & " akta,kesalahan,cetak_notis,no_akaun "
		d = d & " from kompaun.halangan "		
		d = d & " where  tkh_kompaun < to_date('"& tkhs &"','dd/mm/yyyy') and daerah like '"& daerah &"'||'%'  and to_char(tkh_notis1,'yyyy') like'"& tahun &"'||'%'" 
		if tkhd <> "" and tkhh <> ""  then
		d = d & " and tkh_notis1 between to_date('"&tkhd&"','dd/mm/yyyy') and to_date('"&tkhh&"','dd/mm/yyyy')"
		end if
		if kompd <> ""  then
  ...
  <table width="100%" align="center" cellspacing=1>
    <tr align="right" style="font-family: Trebuchet MS; font-size: 10pt;"> 
      ...
      <td colspan="7" > 
        ...
        <a href="hg116.asp?page=1&bilangan=0&ms=pre&ftkhd=<%=tkhd%>&ftkhh=<%=tkhh%>&ftkhn=<%=tkhn%>&ftahun=<%=tahun%>&fkompd=<%=kompd%>&fdaerah=<%=daerah%>&fkomph=<%=komph%>&fp4=Cari"> 
        <img name="firstrec" border="0" src="firstrec.jpg" width="20" height="20" alt="Halaman Mula"></a> 
        <% End If %>
        <% If iPageCurrent <> 1 Then%>
        <a href="hg116.asp?page=<%= iPageCurrent - 1 %>&bilangan=<%=bil-count-iPageSize%>&ms=pre&ftkhd=<%=tkhd%>&ftkhh=<%=tkhh%>&ftkhn=<%=tkhn%>&ftahun=<%=tahun%>&fdaerah=<%=daerah%>&fkompd=<%=kompd%>&fkomph=<%=komph%>&fp4=Cari"> 
        ...
        <a href="hg116.asp?page=<%= iPageCurrent + 1 %>&bilangan=<%=bil%>&ms=next&ftkhd=<%=tkhd%>&ftkhh=<%=tkhh%>&ftkhn=<%=tkhn%>&fkompd=<%=kompd%>&ftahun=<%=tahun%>&fdaerah=<%=daerah%>&fkomph=<%=komph%>&fp4=Cari"> 
        <img name="next" border="0" src="next.jpg" width="20" height="20" alt="Rekod Seterusnya"></a> 
        <% End If 
	  If cint(iPageCurrent) < cint(iPageCount) Then
	  bil = (iPageCount - 1) * iPageSize %>
        <a href="hg116.asp?page=<%=iPageCount %>&bilangan=<%=bil%>&ms=next&ftkhd=<%=tkhd%>&ftkhh=<%=tkhh%>&ftkhn=<%=tkhn%>&fkompd=<%=kompd%>&ftahun=<%=tahun%>&fdaerah=<%=daerah%>&fkomph=<%=komph%>&fp4=Cari"> 
    ...
    <tr bgcolor="<%=color2%>"  style="font-family: Trebuchet MS; font-size: 10pt;"> 
      ...
      <td width="176" align="center"><%=sd("no_akaun")%></td>
