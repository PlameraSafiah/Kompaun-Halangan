     ...
     if tkh2s > tkhs then
		...
		if not Rsh.eof then
           ...
           Set Rsr1 = objConn.Execute(r1)
			...
			s =     "insert into kompaun.halangan (no_kenderaan,nama,kp,alamat1,alamat2,alamat3,tkh_notis1,tkh_undang) "
           	s = s & " values ('"& kereta &"', '"& nama &"', '"& kp &"', "
			s = s & " '"& alamat1 &"', '"& alamat2 &"', '"& alamat3 &"', to_date('"& tnotis &"','ddmmyyyy'), to_date('"& tundang &"','ddmmyyyy'))"
     ...
     if kompaun = ""  then
        ...
        if rssk.eof then
           ...
           response.end
			...
			sk1 =      "select * from kompaun.mahkamah "
			sk1 = sk1 & " where no_kompaun = '"& kompaun &"' "
...
'**************************************************************************************************
  ...
  e = e & " no_kenderaan,masa,to_char(tkh_kompaun,'dd/mm/yyyy') tkompaun, "
...
<%if noresit <> "" then%>
      <tr bgcolor="#CCCCCC"> 
        <td colspan="2" height="11" align="center" bgcolor="<%=color1%>"><b><font face="verdana" size="2">            
            Perhatian!!! </font><font color="#000000">Kompaun Telah Dibayar Pada</font>
            <font color="#FF0000" size="3"><%=tkhbyr%> Bernombor Resit :<%=noresit%> 
            <%end if%></font></b>
        </td>
      </tr>
      <tr> 
  ...
  %>
      ...
      <td bgcolor="<%=color1%>" width="25%"><font  color="#FFFF00">&nbsp;Bayaran 
        ...
        <td bgcolor="#CCCCCC"> 
          <input type="text" name="denda" value="<%=denda%>" size="20" maxlength="100" onFocus="enter('b1save','2');">
        </td>
      </tr>
      <tr> 
